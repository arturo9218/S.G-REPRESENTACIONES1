...
static void aplicarDeratingCalor(unsigned long now) {
  ...
  const bool hot = chipC >= CHIP_TEMP_WARN_C || (!fault1 && tCam >= CAMBIENT_HOT_C);
  if (hot) {
    WiFi.setTxPower(WIFI_POWER_7dBm);
    if (lastWarn == 0 || now - lastWarn >= 90000UL) {
      lastWarn = now;
      pushFlash(FP_CALOR);
      Serial.printf("[CALOR] chip=%.0f sonda=%.1f\n", chipC, tCam);
    }
  } else {
...
static void handleButtonUi() {
  ...
  if (g_uiMode == UI_NORMAL) {
    ...
    if (downEdgeEarly && !upHeld && !setHeld) {
      ...
      refreshNormalDisplay();
      Serial.printf("[BTN] Vista=%d (0=temp 1=fase 2=min)\n", (int)g_normalDisp);
      syncBtnPrevFromPins();
      return;
    }
...
static void networkTask(void *param) {
  ...
  for (;;) {
    esp_task_wdt_reset();
    if (!g_portalRunning && WiFi.status() == WL_CONNECTED) {
      ...
      if (temperatureRead() >= CHIP_TEMP_WARN_C) telEvery = max(telEvery, 120000UL);
      if (now - lastTel >= telEvery) sendNow = true;
      ...
      if (g_pullNowRequested || now - lastPull >= PARAMS_PULL_MS) {
        lastPull = now;
...
void setup() {
  ...
  Serial.println(F("AR Monitoreo PRO400 booteando..."));
  ensureFs();
  loadConfig();
  loadParams();
  syncTelemetryInterval();
  ...
  xTaskCreatePinnedToCore(displayTask, "disp", 2048, NULL, 2, &g_displayTaskHandle, 1);
  setupWifi();
  if (!g_portalRunning && g_uiMode == UI_NORMAL) modoTexto = false;
  xTaskCreatePinnedToCore(networkTask, "net", 8192, NULL, 1, &g_netTaskHandle, 0);
}
