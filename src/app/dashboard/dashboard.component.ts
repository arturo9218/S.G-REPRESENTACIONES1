...
export class DashboardComponent implements OnInit, OnDestroy {
      ...
      if (t1 != null && !inDefrostCycle) {
        if (high != null && t1 >= high) {
          out.push({
            ...
            deviceName: c.name,
            temperatureC: t1,
            message: 'Alarma térmica alta (AR24)',
            detail: `${t1.toFixed(1)} °C (máx. ${high} °C) · ${readingLabel}`,
            kind: 'temp_high',
            severity: 'critical',
          });
          ...
          out.push({
            ...
            deviceName: c.name,
            temperatureC: t1,
            message: 'Alarma térmica baja (AR25)',
            detail: `${t1.toFixed(1)} °C (mín. ${low} °C) · ${readingLabel}`,
            kind: 'temp_low',
            severity: 'warning',
          });
  ...
  private panelAlarmCooldownMsForDeviceAndKind(
    ...
    if (combi) {
      const delayMin = combi.alarmDelayMin;
      const delayMs =
        typeof delayMin === 'number' && Number.isFinite(delayMin) && delayMin > 0
          ? Math.max(minMs, Math.round(delayMin * 60 * 1000))
          : defaultMs;
      return delayMs;
    }
    return defaultMs;
