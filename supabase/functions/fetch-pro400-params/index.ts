// Supabase Edge Function: fetch-pro400-params
// El ESP32 PRO400 obtiene F01–F26 (AR en app) desde `devices.params`.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: row, error } = await supabase
      .from('devices')
      .select('params, updated_at, device_token_hash, equipment_kind')
    ...
    if (!row || row.device_token_hash !== deviceToken) {
      return new Response(JSON.stringify({ error: 'No autorizado o PRO400 no encontrado' }), {
        status: 401,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    return new Response(
      JSON.stringify({
        ...
        equipment_kind: row.equipment_kind ?? null,
      }),
